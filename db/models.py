...
class Song(SQLModel, table=True):
    ...
    # Optional association to stream if you want
    stream_id: Optional[str] = Field(default=None, foreign_key="stream.stream_id")


class Stream(SQLModel, table=True):
    """
    Stream model with persistent admin/participants/blocked lists and queue.
    Stored as JSON columns (works with SQLite/Postgres).
    """
